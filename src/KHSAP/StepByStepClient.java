...
public class StepByStepClient {
	...
	public static void step2ConnectUsingPool() throws JCoException {
		...
		// System.out.println(destination.BACK);
		System.out.println(destination.getAttributes());
		System.out.println();
	}
	...
	public static void step4WorkWithTable() throws JCoException {
		...
		int rownum = codes.getNumRows();
		codes.appendRow();
		codes.lastRow();
		rownum = codes.getNumRows();
	...
	/* 传入参数，返回Table */
	public static void CallRFC() throws JCoException {
		JCoDestination destination = JCoDestinationManager.getDestination(ABAP_AS_POOLED);
		JCoFunction function = destination.getRepository().getFunction("RFC_GET_TABLE_ENTRIES");
		if (function == null) {
			System.out.println("Function is not exeist!");
			throw new RuntimeException("STFC_CONNECTION not found in SAP");
		}
		System.out.println("Function is Found!");
		...
		JCoTable tb = function.getTableParameterList().getTable("ENTRIES");
		...
		for (int i = 0; i < tb.getNumRows(); i++) {
			tb.setRow(i);
			System.out.println(tb.getString("wa") + "\t");
		}
	}

	/* 传入参数Table，返回Table */
	public static void CallRFCTable(String functionName) throws JCoException {
		JCoDestination destination = JCoDestinationManager.getDestination(ABAP_AS_POOLED);
		...
		for (int i = 0; i < 3; i++) {
			tb.appendRow();
			tb.lastRow();
			// tb.getValue(0).toString()
			//设置属性值
			tb.setValue("WA", i);
//			tb.setValue("Field_2", String.valueOf(i));
		}
//
//		try {
//			function.execute(destination);
...
//		}
		...
		// 获取返回值
		tb = function.getTableParameterList().getTable("ENTRIES");
		...
		for (int i = 0; i < tb.getNumRows(); i++) {
			tb.setRow(i);
			System.out.print(tb.getString("WA") + "\t");
//			System.out.print(tb.getString("Field_3") + "\t");
			System.out.print("\n");
		}
	}
	...
	public static void main(String[] args) throws JCoException {
		...
		// JCoDestinationManager.getDestination(ABAP_AS_POOLED);
		// System.out.println(destination.getDestinationName());
		// System.out.println(destination.getExpirationCheckPeriod());

		ConnectPooled pool = new ConnectPooled();
		// step1Connect();
		// 连接池测试
		 step2ConnectUsingPool();//("RFC_GET_TABLE_ENTRIES");
		 CallRFCTable("RFC_GET_TABLE_ENTRIES");
//		CallRFC();
		// step3SimpleCall();
		// step4WorkWithTable();
